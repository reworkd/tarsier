...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("svg:rect#my-rect.class1.class2", '*[name()="svg:rect"]#my-rect.class1.class2'),
...
async def test_fixNamespaces(browser_adapter_with_js, input_tag, expected_output):
    ...
    assert result == expected_output, f"For {input_tag}, expected {expected_output} but got {result}"


@pytest.mark.asyncio
async def test_xpath_namespace(tarsier, async_page):
    html_with_namespace_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "mock_html/namespace.html"))
